...
> **Banco definido pela Alphractal** em 18/08/2026 — *"salva no banco deles, ClickHouse"* ([doc 10](./10-registro-respostas-parceiro.md)). A análise anterior — que recomendava SQLite — está preservada na seção 6 como registro da decisão, mas não vale mais. Contexto da stack completa em [09 — Arquitetura](./09-arquitetura-e-stack.md).
>
> ⚠️ **Onde o banco roda:** o protótipo usa **ClickHouse local via Docker**, com schema espelhando o padrão da Alphractal. Gravar na instância de produção deles colidiria com a restrição do TAP (*"sem integração direta em produção"*). Ver [09 §7](./09-arquitetura-e-stack.md) e dúvida 21.
...
**Como resolvemos:** o spool de arquivos NDJSON descrito em [09 §2](./09-arquitetura-e-stack.md) agrupa **~25 blocos por lote** (arquivo fechado a cada 5 minutos), resultando em ~288 `INSERT`s/dia.

> **Por que 5 minutos e não 1.** Um arquivo por minuto — a proposta anterior — daria ~5 blocos por lote e **1.440 parts/dia**. Não é o caso patológico dos 7.200, mas continua sendo um padrão que o ClickHouse não gosta: o merge acompanha, porém sem folga. Como o spool é **caminho frio**, atrasar o lote em 5 minutos não custa absolutamente nada em latência percebida — o painel ao vivo nunca lê daqui (RN-14) — e reduz os parts em 5×. É a troca mais barata do projeto.
...
| **Retenção declarativa** | `TTL block_time + INTERVAL 30 DAY` na definição da tabela. A **RN-15** vira configuração, não job agendado |
...
| `TTL 30 DAY` | Retenção da RN-15 aplicada pelo próprio banco. Cobre a janela de 30 dias exigida pelo D-02 |
...
### 3.3 Agregados diários (materialized view)

O parceiro indicou **granularidade diária** como frequência de análise (*"nível diário, time frame indeterminado"*, 18/08 — [doc 10 §3.1](./10-registro-respostas-parceiro.md)). O rollup diário existe para atender isso: sobrevive ao `TTL` de 30 dias dos dados brutos e serve as visões de longo prazo sem varrer nada.

```sql
CREATE TABLE fee_stats_daily
(
    day             Date,
    avg_base_fee    AggregateFunction(avg, UInt64),
    min_base_fee    AggregateFunction(min, UInt64),
...
CREATE MATERIALIZED VIEW fee_stats_daily_mv TO fee_stats_daily AS
SELECT
...
> A mesma advertência da seção anterior vale aqui: **MV é gatilho de inserção**. Criar `fee_stats_daily_mv` **antes** da primeira carga, junto com a horária.

**O tempo real não muda por causa disso.** A resposta do parceiro sobre "nível diário" foi interpretada como granularidade de *análise*, não de *atualização do painel* — que continua governada pelo RNF-01 (< 2 s). Se essa leitura estiver errada, o escopo do projeto muda; é a dúvida 27.

### 3.4 Consulta do percentil (D-02)
...
| ID | Texto revisado |
|---|---|
| RF-34 | Persistir blocos **em lote** (~25 registros ou 300 s por `INSERT`), nunca linha a linha |
...
| RN-15 | Retenção por `TTL` declarativo — **30 dias** de dado bruto; agregados horários e diários indefinidamente |
...
| # | Pendência | Estado |
|---|---|---|
| 21 | Instância de ClickHouse: deles ou Docker local? Schema e convenção de nomes a seguir? | **Aberta** — decisão provisória em [09 §7](./09-arquitetura-e-stack.md) |
| 24 | Padrão de ETL em Python (orquestrador, convenções) | Encerrada em 18/08 — pipeline próprio deles |
| 27 | *"Nível diário"* refere-se à análise ou à atualização do painel? | **Aberta** — bloqueante, ver [10 §3.1](./10-registro-respostas-parceiro.md) |
